...
if (cluster.isMaster) {
	logger.info(`connect-proxy@${VERSION} master process started with pid ${process.pid}`);
	if (VPN_CONNECT_INSTANCE_COUNT > 1) {
		logger.info(`spawning ${VPN_CONNECT_INSTANCE_COUNT} proxy worker processes`);
		// spawn worker processes
		_.times(VPN_CONNECT_INSTANCE_COUNT, cluster.fork);
		cluster.on('exit', (worker: cluster.Worker, code: number) => {
			logger.error(`proxy worker ${worker.process.pid} exited with code ${code}`);
			cluster.fork();
		});
	}
}
